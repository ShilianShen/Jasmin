...
static bool TEMPO_CreatePage_RK(Page* page, const cJSON* page_json) {
    ...
    if (cJSON_ExistKey(page_json, key = "anchor")) {
        REQ_CONDITION(cJSON_LoadByKey(page_json, key, JSM_INT, &page->anchor, NULL), return false);
    }
    if (cJSON_ExistKey(page_json, key = "src")) {
        REQ_CONDITION(cJSON_LoadByKey(page_json, key, JSM_FRECT, &page->src_rect, NULL), return false);
        page->src = &page->src_rect;
    }
    if (cJSON_ExistKey(page_json, key = "color")) {
        REQ_CONDITION(cJSON_LoadByKey(page_json, key, JSM_COLOR, &page->color, NULL), return false);
    }
    return true;
}
